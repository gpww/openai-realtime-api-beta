...
/**
 * Valid audio formats
 * @typedef {"Raw8KHz16BitMonoPcm"|Raw16KHz16BitMonoPcm"|"Raw44100Hz16BitMonoPcm"|"MonoMp3"} AudioFormatType
 */
 ...
 * @property {"input_text"| "tts_text"} type
...
export class RealtimeClient extends RealtimeEventHandler {
  ...
  async connect({ model, userId } = { model: 'gpt-4o-min', userId: '' }) {
    ...
    await this.realtime.connect({ model, userId });
    // this.updateSession();
    return true;
  }
  ...
  appendInputAudio(arrayBuffer) {
    if (arrayBuffer.byteLength > 0 && this.realtime.isConnected()) {
