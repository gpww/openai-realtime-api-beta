...
export class RealtimeAPI extends RealtimeEventHandler {
   ...
   */
  async connect({ model, userId } = { model: 'gpt-4o-min', userId: '' }) {
    ...
    // 新增：先用 HTTPS 校验
    if (typeof fetch === 'function' && this.apiKey) {
        // 直接用 GET 请求 /realtime 进行校验
        const httpUrl = this.url.replace(/^wss?:/, 'https:');
        const params = new URLSearchParams({ model, userId }).toString();
        const resp = await fetch(`${httpUrl}?${params}`, {
          method: 'GET',
          headers: {
        ...
        if (!resp.ok) {
          ...
          console.log('连接失败:', resp.status, errText);
          throw new Error('连接失败', resp.status, errText);
        }
      }
    ...
    if (globalThis.WebSocket) {
      ...
      const wsUrl = `${this.url}?model=${model}&userId=${userId}`;
      let ws;
      try {
      ...
      return new Promise((resolve, reject) => {
        ...
        ws.addEventListener('close', connectionErrorHandler);
