...
export class RealtimeConversation {
  ...
  EventProcessors = {
    ...
    'conversation.item.input_audio_transcription.delta': (event) => {
      ...
      // Check if the last item is a user message with in_progress status
      let item = null;
      ...
      if (!item) {
        // Create a new item if no in-progress user message exists
        const newItem = {
          id: 'tempId',
          type: 'message',
          role: 'user',
        ...
        this.itemLookup[item_id] = newItem;
        this.items.push(newItem);
        item = newItem;
      } else {
        // Update existing item - find the last audio content
        let audioContentIndex = -1;
        for (let i = item.content.length - 1; i >= 0; i--) {
          if (item.content[i].type === 'input_audio') {
            audioContentIndex = i;
            break;
          }
        }
        ...
        if (audioContentIndex >= 0) {
          item.content[audioContentIndex].transcript = transcript;
        }
    ...
    'conversation.item.input_audio_transcription.completed': (event) => {
      ...
      if (this.items.length > 0) {
        ...
        if (lastItem.type === 'message' && 
            ...
            lastItem.status === 'in_progress') {
          item = lastItem;
          // Update the item_id to match the event
          item.id = item_id;
          // Update the lookup
          delete this.itemLookup[Object.keys(this.itemLookup).find(key => this.itemLookup[key] === item)];
          this.itemLookup[item_id] = item;
        }
      }
      ...
      if (!item) {
        ...
        // Find the last audio content in the user message
        let audioContentIndex = -1;
        for (let i = item.content.length - 1; i >= 0; i--) {
          if (item.content[i].type === 'input_audio') {
            audioContentIndex = i;
