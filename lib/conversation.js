...
export class RealtimeConversation {
  ...
  EventProcessors = {
    ...
    'conversation.item.input_audio_transcription.delta': (event) => {
      ...
      if (!item) {
        // Create a new item if it doesn't exist (VAD mode scenario)
        const newItem = {
          id: item_id,
          type: 'message',
          role: 'user',
      ...
      // Since we either found an existing item with audio content or created one,
      // audioContentIndex should always be valid here
      if (audioContentIndex >= 0) {
        // Set the delta transcript (replace, not append for delta events)
